...
### Elementi che hanno reso possibili i Container:   


Uno dei concetti fondamentali dei container è l'isolamento dei processi!   

**`chroot`**:  
chroot serviva a cambiare la cartella principale (`/`) per un programma, ma era insicuro e facilmente aggirabile da attaccanti.   


**FreeBSD Jails e VServer**:  
Le Jails (introdotte da freeBSD) risolvono i buchi di `chroot` e creano un isolamento più profondo e sicuro

- Network: Invece di usare l'IP del server principale, la Jail ha il suo indirizzo IP virtuale (Se il server si chiama 192.168.1.10, la Jail può avere 192.168.1.11. Se un hacker attacca l'IP .11, non tocca il .10) 

- Process list: Dentro la jail si vedono solo i processi che girano in quell'ambiente! non si possono vedere gli altri processi sul sistema  

- Hostname: La Jail ha il suo hostname diverso dal hostname del server

- Users & Root: La Jail ha il suo file delle password e degli utenti, l'utente amministratore dentro la jail ha i pieni poteri ma solamente sui file dentro la jail, se cerca di fare qualcosa fuori dalla jail il kernel lo blocca.  

Le Jails erano sicure ma non avevano limiti hardware $\rightarrow$ esposte ad attacchi DoS.  
**VServer** inventa il *Resource Partitioning*, che vincola le risorse HW che una jail può usare!   



**`namespaces`**:   
I namespaces limitano ciò che un processo può vedere.  

Esistono 7 categorie di namespaces:  
1. PID namespace: impedisce di vedere gli altri processi
2. NET namespace: impedisce di vedere la vera scheda di rete 
3. MNT namespace: impedisce di vedere le vere cartelle del disco fisso
n. (e così via per utenti, hostname, ecc...)

Es. User namespace isola gli utenti ed è ciò che permette a Docker di fare "User Mapping", ossia permettere a un utente di essere root dentro al container, quando in realtà tale user nel host server principale è un utente `nobody`. Questo garantisce che nel caso in cui venga bucato il container, l'utente non ha nessun potere sul host.  



**`cgroups`**:   
I cgroups limitano le risorse che un processo può usare, essi monitorano e tagliano 4 cose principali: 

1. RAM: quanti MB può occupare il processo
2. CPU: quanti cicli di processore o quanti core può usare 
3. Network I/O: quanta banda internet puoi occupare (es. 10Mb/s)  
4. Disk I/O: quanto velocemente puoi leggere / scrivere sul disco  

Es. creiamo un container con docker che crea un cgroup e imposta la regola di limite di 500MB di RAM.  
VIene creato il processo e inizia a lavorare, mettiamo caso che a un certo punto arrivi un picco di traffico e il processo chieda alla RAM 501MB.  
A questo punto interviene il cgroup che uccide il processo (OOM Killer) mantenendo il server host principale fluido, vivo e reattivo; le altre app / processi non si accorgono di niente.   

Quando viene chiesta più memoria RAM di quella messa a disposizione si preferisce quasi sempre terminare il processo con OOM Killer per evitare il rallentamento dell'app tramite swapping (che andrebbe comunque specificata come opzione nel file di config.)  

Questo perchè la RAM è una risorsa **INCOMPRIMIBILE**:  
È uno spazio fisico, se un programma deve salvare in memoria una foto da 2MB e ha solo 1MB libero non può 'aspettare', i dati non ci stanno fisicamente.  
Normalemente i programmi quando richiedono più spazio di quello a disposizione, crashano da soli in modo disastroso, possono perfino corrempere i dati.  
L'OOM Killer del cgroup interviene prima che il programma crashi, lo spegne lui per evitare che faccia danni.   
...
> come mai se un container inizia a fare swapping rallenta l'intero server? il fatto che debba salvare i dati sul disco (fare swapping) e fare l'eviction dalla RAM non dovrebbe rallentare solo lui? se ho messo che può usare il x% di CPU allora rimane con quel x% e l'operazione di swapping sarà lenta ma non userà più del x% della CPU che ho imposto con i cgroups , o si? quindi come mai rallenta l'intero server e tutti gli altri container ?