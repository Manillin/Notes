...
- **Runtime Library**: funzioni utili  
    ...
    - `omp_thet_num_threads()`: quanti thread ci sono in totale 
...
- `pragma omp single`:  
    ...
    - **attenzione**: C'è una barriera esplicita alla fine di questo blocco! gli altri thread saltano il blocco e si aspettano che il thread 'prescelto' finisca il suo lavoro (spesso single si usa per caricare dati condivisi che servono anche agli altri), c'è comunque la possibilità di usare la `nowait`.  
