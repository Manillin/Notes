...
tassonomia: disciplina che si occupa della classificazione gerarchica di elementi relativi a un determinato settore 
...
3. **SIMD** (Single Instruction Multiple Data stream)  
    ...
    Mi permette di eseguire la stessa operazione su dati differenti, consiste nel più classico tipo di data parallelism (architettura delle GPU). 
...
Nel panorama moderno le CPU sono MIMD a più core, ma ogni core è capace comuque di fare istruzioni SIMD per allegerire i calcoli quando serve.  
